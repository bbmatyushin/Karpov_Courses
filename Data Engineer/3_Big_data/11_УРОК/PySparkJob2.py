...
def process(spark, flights_path, result_path):
    ...
    df = data_flights.where(data_flights['TAIL_NUMBER'].isNotNull()) \
        .groupBy(data_flights['ORIGIN_AIRPORT'],
                 data_flights['DESTINATION_AIRPORT']) \
        .agg(F.count(data_flights['TAIL_NUMBER']).alias('tail_count'),
             F.avg(data_flights['AIR_TIME']).alias('avg_air_time')) \
